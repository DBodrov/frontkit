...
export function HelpIcon({ dataTestId = 'input-search-icon', text, colorized = false }: HelpIconProps): JSX.Element {
    const [isOpen, setIsOpen] = React.useState(false);
    const theme = React.useContext(ThemeContext);
    const element = React.useRef<HTMLDivElement>(null);
    const hideTooltip = React.useCallback(() => setIsOpen(false), [setIsOpen]);
    const showTooltip = React.useCallback(() => setIsOpen(true), [setIsOpen]);
    const onClick = React.useCallback(
        (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
            stopPropagation(e);
            showTooltip();
        },
        [showTooltip, stopPropagation],
    );
    return (
        <div
        ...
        >
            ...
            >
                <circle cx="10.5" cy="10.5" r="10.5" fill={colorized ? theme.mainColor : '#C6CED9'} />
                <path
                    ...
                    fill={colorized ? theme.btnTextColor : '#8C929C'}
                />
            </svg>
